...
function Team() {
  ...
  const fetchTeamCount = useCallback(async () => {
    try {
      const res = await axios.get("tasks/teams/count/");
      setTeamCount(res.data.team_count || 0);
    } catch (error) {
      console.error("Error fetching team count", error);
    }
  ...
  const fetchTeamList = useCallback(async () => {
    try {
      const res = await axios.get("tasks/teams/all/");
      setTeams(res.data || []);
      setTeamCount(res.data.length || 0);
    } catch (error) {
      console.error("Error fetching teams", error);
  ...
  const handleTeamCreate = async (e) => {
    e.preventDefault();
    try {
      await axios.post("tasks/teams/create/", newTeam);
      alert("Team created successfully!");
      setNewTeam({ name: "", members: [] });
      setShowCreateModal(false);
      fetchTeamList();
  ...
  const handleTeamDelete = async (teamId) => {
    if (!window.confirm("Are you sure you want to delete this team?")) return;
    try {
      await axios.delete(`tasks/${teamId}/delete/`);
      fetchTeamList();
    } catch (error) {
      console.error("Error deleting team", error);
      alert("Failed to delete team.");
