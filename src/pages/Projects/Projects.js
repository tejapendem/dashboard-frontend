...
import React, { useEffect, useState, useCallback, useContext } from "react";
import axios from "../../api/axios"; // ✅ use instance, not base axios
import { ThemeContext } from "../../context/ThemeContext";
import "./Projects.css";
...
function Projects() {
  ...
  const fetchTasks = useCallback(async () => {
    try {
      const response = await axios.get("tasks/projects/");
      const data = response.data || {};
      setWorking(data.working || []);
      setInProgress(data.in_progress || []);
      setCompleted(data.completed || []);
  ...
  const handleStatusChange = async (taskId, newStatus) => {
    try {
      await axios.patch(`tasks/${taskId}/update_status/`, { status: newStatus });
      fetchTasks(); // refresh UI
    } catch (error) {
      console.error("Failed to update task status:", error);
      alert("Status update failed.");
  ...
  const handleDeleteTask = async (taskId) => {
    ...
    try {
      await axios.delete(`tasks/tasks/${taskId}/delete/`);
      fetchTasks(); // refresh task list
    } catch (error) {
      console.error("Failed to delete task:", error);
      alert("Failed to delete task.");
